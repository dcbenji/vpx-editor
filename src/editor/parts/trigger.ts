...
interface TriggerItem {
  ...
  rotation?: number;
  material?: string;
  drag_points?: Array<{ vertex?: Point; x?: number; y?: number }>;
...
function renderTriggerShape(
  ...
  if (item.drag_points && item.drag_points.length >= 3) {
    let vertices: Point[];
    if (shape === 'none') {
      vertices = item.drag_points.map(p => getDragPointCoords(p));
    } else {
      const result = generateSmoothedPath(item.drag_points, true, PATH_SMOOTHING_ACCURACY);
...
export function uiRenderPass1(item: TriggerItem, _isSelected: boolean): void {
...
export function uiRenderPass2(item: TriggerItem, isSelected: boolean): void {
  if (!elements.ctx) return;
  const ctx = elements.ctx;
...
export function renderBlueprint(ctx: CanvasRenderingContext2D, item: TriggerItem, scale: number, solid: boolean): void {
  const center = item.center || item.vCenter;
  if (!center) return;
...
export function render(item: TriggerItem, isSelected: boolean): void {
  uiRenderPass1(item, isSelected);
  uiRenderPass2(item, isSelected);
}

export function hitTestTrigger(
  item: TriggerItem,
  worldX: number,
  worldY: number,
  _center: Point | undefined,
  distFromCenter: number | undefined
): boolean {
  const shape = (item.shape || 'wire_a').toLowerCase();
  if (shape === 'star' || shape === 'button') {
    return (distFromCenter ?? 0) < (item.radius ?? TRIGGER_DEFAULTS.radius);
  }
...
export function triggerProperties(item: TriggerItem): string {
  return `
    <div class="prop-tabs">
...
function getCenter(item: TriggerItem): Point | null {
  return item.center ? { x: item.center.x, y: item.center.y } : null;
}

function putCenter(item: TriggerItem, center: Point): void {
  item.center = { x: center.x, y: center.y };
}

const triggerRenderer: IEditable = {
  render,
  uiRenderPass1,
  uiRenderPass2,
  renderBlueprint,
  hitTest: hitTestTrigger,
  create3DMesh: createTrigger3DMesh,
  getProperties: triggerProperties,
  getCenter,
  putCenter,
};

registerEditable('Trigger', triggerRenderer);
