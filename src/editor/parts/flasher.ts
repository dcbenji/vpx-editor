...
function getFlasherVertices(item: Flasher): FlasherVertex[] {
  let points: DragPoint[] = item.drag_points || [];
  ...
  if (points.length < 3) {
    ...
    points = [
      { vertex: { x: cx - halfSize, y: cy - halfSize } },
      { vertex: { x: cx - halfSize, y: cy + halfSize } },
      { vertex: { x: cx + halfSize, y: cy + halfSize } },
      { vertex: { x: cx + halfSize, y: cy - halfSize } },
    ];
...
export function uiRenderPass1(item: Flasher, _isSelected: boolean): void {
...
export function render(item: Flasher, isSelected: boolean): void {
  uiRenderPass1(item, isSelected);
  uiRenderPass2(item, isSelected);
}

export function hitTestFlasher(item: Flasher, worldX: number, worldY: number): boolean {
...
function getStyleOptions(mode: string, selectedStyle: number | undefined): string {
...
function getCenter(item: Flasher): Point | null {
  const points = item.drag_points;
  ...
  if (item.drag_points && item.drag_points.length > 0) {
    for (const p of item.drag_points) {
      if (p.vertex) {
        ...
        (p as unknown as { x: number; y: number }).x += dx;
        (p as unknown as { x: number; y: number }).y += dy;
      }
    }
  }
  if (item.pos_x !== undefined) item.pos_x += dx;
  if (item.pos_y !== undefined) item.pos_y += dy;
}

const flasherRenderer: IEditable = {
  render,
  uiRenderPass1,
  uiRenderPass2,
  renderBlueprint,
  hitTest: hitTestFlasher,
  create3DMesh: createFlasher3DMesh,
  getProperties: flasherProperties,
  getCenter,
  putCenter,
};

registerEditable('Flasher', flasherRenderer);
